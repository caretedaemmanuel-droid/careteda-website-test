  ...
  template: `
    <section class="min-h-screen bg-black text-white pt-28 pb-20 px-4 md:px-8">
      <div class="max-w-[800px] mx-auto" *ngIf="post">
        ...
        <p
          *ngIf="post.subtitle"
          class="text-white/95 text-base md:text-lg italic mb-6 leading-relaxed"
        >
          {{ post.subtitle }}
        </p>
        <div class="mb-8">
        ...
        <div class="mb-10 text-white/85 text-sm space-y-1">
          <p>{{ post.date }}</p>
          <p>
            <span *ngIf="post.wordCount">{{ post.wordCount }} · </span>{{ post.readTime }}
          </p>
        </div>

        <!-- Key Takeaways box -->
        <div
          *ngIf="post.keyTakeaways && post.keyTakeaways.length"
          class="rounded-xl bg-[#0d1f3c] border border-[#2563EB]/50 p-6 md:p-8 mb-10"
        >
          <h3 class="text-white font-bold text-xl mb-5">Key Takeaways</h3>
          <ul class="space-y-4">
            <li
              *ngFor="let item of post.keyTakeaways"
              class="text-white/95 text-base leading-relaxed flex gap-3 items-start"
            >
        ...
        <div class="space-y-6">
          <div *ngFor="let section of post.sections" class="space-y-4">
            <h2 *ngIf="section.heading" class="text-xl md:text-2xl font-bold text-white mt-10">
              {{ section.heading }}
            </h2>
            ...
            <ng-container *ngIf="section.blocks && section.blocks.length">
              <ng-container *ngFor="let block of section.blocks">
                <p *ngIf="block.type === 'p'" class="text-white/95 text-base leading-relaxed">
                  {{ block.text }}
                </p>
                <ul *ngIf="block.type === 'ul'" class="space-y-2 pl-4">
                  <li
                    *ngFor="let item of block.items"
                    class="text-white/95 text-base leading-relaxed flex gap-2"
                  >
                ...
                <p
                  *ngIf="block.type === 'caption'"
                  class="text-white/60 text-sm leading-relaxed italic -mt-2 mb-4"
                >
                  {{ block.text }}
                </p>
                <p *ngIf="block.type === 'ref'" class="text-white/95 text-base leading-relaxed">
                  ...
                  <a
                    *ngIf="block.href"
                    ...
                    class="text-[#60A5FA] underline underline-offset-2 break-all hover:text-white transition-colors"
                    >{{ block.linkText || block.href }}</a
                  >
                </p>
                <p
                  *ngIf="block.type === 'html'"
                  class="text-white/95 text-base leading-relaxed"
            ...
            <ng-container *ngIf="!section.blocks || !section.blocks.length">
              ...
              <p
                *ngFor="let para of section.paragraphs"
                class="text-white/95 text-base leading-relaxed"
              >
                {{ para }}
              </p>
              <ul *ngIf="section.bullets" class="space-y-2 pl-4">
                <li
                  *ngFor="let bullet of section.bullets"
                  class="text-white/95 text-base leading-relaxed flex gap-2"
                >
...
export class BlogDetailComponent implements OnInit {
  ...
  private posts: { [slug: string]: BlogPost } = {
      ...
      sections: [
        ...
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: `Human engineers are remarkably good at reading ambiguous specs correctly, and remarkably bad at noticing they're doing it. Years of tribal knowledge, hallway conversations, and "that's just how we always do resets here" quietly fill every gap in a document before an engineer ever opens an editor. The ambiguity doesn't disappear. It just gets resolved by a human brain fast enough that nobody notices a decision was made at all.`,
            },
            {
              type: 'p',
              text: `An agent has none of that tribal context, and unlike a junior engineer, it rarely asks a clarifying question. A study cataloguing errors in LLM-generated RTL sorted the recurring ambiguity failures into a few clear buckets,[2] and each one maps to a real gap in how specs get written today:`,
            },
            {
              type: 'ul',
              items: [
            ...
            {
              type: 'p',
              text: `None of these are exotic edge cases. They are the ordinary texture of how specifications get written by people who already know what they mean and are writing for other people who will fill in the rest.`,
            },
            {
              type: 'img',
              src: 'assets/blogs/spec-bottleneck-ambiguity.png',
              alt: 'Three ways a specification can become ambiguous',
            },
            {
              type: 'caption',
              text: 'Figure 1. The three recurring ambiguity classes behind LLM-generated RTL errors.',
            },
          ],
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: `It's tempting to treat this as a model-capability problem - wait for the next generation of LLMs and the ambiguity will resolve itself. But ambiguity in a spec isn't a knowledge gap the model is missing; it's information that was never in the document in the first place. No amount of additional model capability can recover a decision that no one made.`,
            },
            {
              type: 'p',
        ...
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: `If ambiguity lives in the spec and not in the model, then the highest-leverage place to intervene in an agentic chip design flow isn't the generation step at all. It's the step before it - the one that turns a prose document, however well written, into something with only one possible reading.`,
            },
            {
              type: 'p',
            ...
            {
              type: 'p',
              text: `None of this is about distrusting the model. It's about recognizing that a model faithfully executing an ambiguous instruction will produce a faithfully ambiguous result - confidently, consistently, and without raising its hand. The fix isn't a smarter executor. It's a spec that doesn't need one.`,
            },
            {
              type: 'p',
        ...
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: 'CaretEDA builds AI-native design systems for modern semiconductor teams, with its Spec-to-Netlist™ platform serving as a powerful force multiplier across the entire engineering workflow. To learn more or request a demo, contact info@careteda.com',
            },
          ],
        },
        {
          ...
          blocks: [
            {
              type: 'ref',
              text: '[1] A Critical Review and Evaluation of LLMs for RTL Generation. ',
              href: 'https://www.researchgate.net/publication/400927513_A_Critical_Review_and_Evaluation_of_LLMs_for_RTL_Generation',
            },
            {
              type: 'ref',
              text: '[2] Understanding and Mitigating Errors of LLM-Generated RTL Code. arXiv:2508.05266. ',
              href: 'https://arxiv.org/pdf/2508.05266',
            },
            {
              type: 'ref',
              text: '[3] Spec2RTL-Agent: Automated Hardware Code Generation from Complex Specifications Using LLM Agent Systems. arXiv:2506.13905. ',
              href: 'https://arxiv.org/pdf/2506.13905',
            },
            {
              type: 'ref',
              text: '[4] Proof2Silicon: Prompt Repair for Verified Code and Hardware Generation via Reinforcement Learning. arXiv:2509.06239. ',
              href: 'https://arxiv.org/pdf/2509.06239',
            },
            {
              type: 'ref',
              text: '[5] Cosler, M., Hahn, C., Mendoza, D., Schmitt, F., and Trippel, C. nl2spec: Interactively Translating Unstructured Natural Language to Temporal Logics with Large Language Models. CAV 2023, arXiv:2303.04864. ',
              href: 'https://arxiv.org/abs/2303.04864',
            },
            {
              type: 'ref',
              text: '[6] SIGARCH Blog: Interactively Translating Unstructured Natural Language to Temporal Logics with nl2spec. ',
              href: 'https://www.sigarch.org/interactively-translating-unstructured-natural-language-to-temporal-logics-with-nl2spec/',
            },
          ],
        },
      ],
    ...
    verilator: {
      ...
      summary:
        "How a tool born at Digital Equipment Corporation in 1994 ended up at the center of chip verification for some of the world's most demanding silicon.",
      sections: [
        {
          paragraphs: [
    ...
    'code-generation': {
      ...
      summary:
        'LLMs can produce RTL that compiles and simulates — but hardware is a contract with physics, and physics does not accept plausible drafts.',
      keyTakeaways: [
      ...
      sections: [
        ...
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: 'Power, performance, and area are not secondary metrics. They are the outcome that determines whether a chip ships.',
            },
            ...
            { type: 'p', text: '…and still be unusable because it:' },
            {
              type: 'ul',
              items: [
                'Exceeds the target area by 2–4X',
                'Violates power envelope constraints',
                'Forces micro-architectural rework at the synthesis or place-and-route stage',
              ],
            },
            {
              type: 'p',
              text: 'This is not a failure of intelligence. It is a failure of objective alignment.',
            },
            {
              type: 'p',
              text: 'Experienced hardware engineers internalize PPA tradeoffs instinctively. Models do not—unless those tradeoffs are explicitly encoded into the generation pipeline.',
            },
          ],
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: 'Hardware design is governed by accumulated domain knowledge that is rarely explicit in code:',
            },
            {
              type: 'ul',
              items: [
                'What constitutes a safe clock domain crossing',
                'Which constructs are synthesizable versus simulation-only',
                'How reset sequencing interacts with power domain boundaries',
                'How bus protocols (AXI, AHB, PCIe) behave under backpressure, stall, and error-injection conditions',
              ],
            },
            { type: 'p', text: 'These are not stylistic conventions. They are invariants.' },
            {
              type: 'p',
              text: 'LLMs trained on mixed HDL corpora do not inherently distinguish between:',
            },
            {
              type: 'ul',
              items: [
                'Behavioral examples and production RTL',
                'FPGA-friendly constructs and ASIC-safe ones',
                'Tutorial code and million-line SoC design patterns',
              ],
            },
            {
              type: 'p',
              text: 'Without embedded domain priors, a model will generate RTL that looks right and simulates cleanly—and still cannot be manufactured. This is why prompt engineering alone is insufficient. Hardware generation requires structural constraints baked into the generation process, not applied afterward.',
            },
          ],
        },
        {
          ...
          blocks: [
            ...
            {
              type: 'p',
              text: 'In software, a bad suggestion costs a failed test. In hardware, the same mistake can cost:',
            },
            {
              type: 'ul',
              items: [
                'Weeks of synthesis and place-and-route churn',
                'A metal-layer engineering change order (ECO)',
                'Or a full tape-out re-spin',
              ],
            },
            {
              type: 'p',
              text: 'That asymmetry changes the rules. Hardware flows demand correct-by-construction properties:',
            },
            {
              type: 'ul',
              items: [
                ...
                'No illegal clock domain crossing (CDC) paths',
                'No ambiguous reset semantics',
              ],
            },
            {
              type: 'p',
              text: 'These are not things you "test into correctness." They must be structurally impossible. An LLM that emits RTL without enforcing these invariants is not generating hardware. It is generating latent risk.',
            },
          ],
        },
        {
          ...
          blocks: [
            ...
            {
              type: 'p',
              text: 'Two RTL implementations that are functionally identical can differ dramatically in:',
            },
            {
              type: 'ul',
              items: [
                ...
                'Sensitivity to PVT (process, voltage, temperature) corners',
              ],
            },
            { type: 'p', text: 'LLMs do not reason about:' },
            {
              ...
              items: [
                'Critical paths',
                'Fanout-induced delay and buffer-tree implications',
                'Retiming opportunities',
                'Physical locality after placement',
              ],
            },
            {
              type: 'p',
              text: 'Yet these factors determine whether a design closes timing—or fails weeks before tape-out.',
            },
            {
              type: 'p',
              text: 'Hardware generation without timing awareness is fundamentally misaligned with the realities of physical implementation.',
            },
          ],
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: 'The hardest problem in hardware is not writing RTL. It is ensuring the RTL means what the specification meant.',
            },
            ...
            { type: 'p', text: 'A single misinterpreted sentence can result in:' },
            {
              type: 'ul',
              items: [
                'A dropped corner case in a state machine or handshake sequence',
                'A protocol violation under rare conditions',
                'A silicon bug that escapes simulation and is only caught during post-silicon validation—or in the field',
              ],
            },
            {
              type: 'p',
              text: 'Verifying spec-to-RTL fidelity requires formal methods, exhaustive testbenches, and equivalence checking across abstraction levels. Plausible interpretations are not enough. Hardware demands provable ones.',
            },
          ],
        },
        {
          ...
          blocks: [
            {
              type: 'p',
              text: 'SystemVerilog generation is tractable because it reduces to statistical language modeling—predicting likely token sequences from training corpora.',
            },
            ...
            {
              type: 'p',
              text: 'The future belongs to tools that internalize this distinction—encoding physical constraints, PPA objectives, and verification requirements directly into the generation pipeline rather than bolting them on after the fact.',
            },
            {
              type: 'p',
              text: 'LLMs are powerful generative engines. But silicon does not ship on plausibility. It ships on correctness—provable, synthesizable, timing-clean correctness.',
            },
            {
              type: 'p',
              text: 'The question is not whether AI can write SystemVerilog. It is whether AI can deliver hardware. That requires teams willing to deeply examine their existing flows, identify and prioritize optimization opportunities, understand real user behaviors, and enable agentic workflows that sustain a competitive edge.',
            },
          ],
        },
        {
    ...
    'silicon-imperative': {
      ...
      subtitle:
        'Rising Chip Design Complexity, Shrinking Tapeout Cycles, and the AI Transformation of System Design',
      date: 'March 2026',
      readTime: '5 min read',
      wordCount: '1,000 words',
      ...
      summary:
        'Rising chip design complexity, shrinking tapeout cycles, and why the $775B semiconductor market is making AI-native design a strategic necessity.',
      keyTakeaways: [
